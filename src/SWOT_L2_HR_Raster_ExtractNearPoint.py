...
try:
    bounding_LUT = pd.read_csv(base_dir / 'config'/ 'aoi.csv')
...
for g in gs:
    ...
    for product in swot_products:
        ...
        output1 = gauge_folder / ('%s_%s_%s_%skm_%s_%s.geojson' %(aoi,product,l2_processing,search_radius_hr,gauge_name,types))
