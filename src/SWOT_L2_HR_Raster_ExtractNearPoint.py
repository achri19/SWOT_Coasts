...
from swot_utils import (haversine_vectorized, fix_latlon
...
parser.add_argument('--limited', dest='limited', action="store_true",   help='Limited will only save a few attributes, other will save all attributes')
...
if l2version == 'C':
    L2HR_folder = base_dir / 'Data' / (l2hr_prefix + '_2.0') / aoi / 'Extracted'
else:
    L2HR_folder = base_dir / 'Data' / (l2hr_prefix + '_' + l2version)/ aoi / 'Extracted'
...
try:
   if args.limited:
       ref_l2 = glob.glob(str(L2HR_folder / ('%s_%s_*_limited.nc' %(l2hr_prefix,resolution))))[0]
   else:
       ref_l2 = glob.glob(str(L2HR_folder / ('%s_%s_*.nc' %(l2hr_prefix,resolution))))[0]
...
variables_to_remove = ['spatial_ref']#,'wind_speed_rad','rad_surface_type_flag','rad_tmb_187','rad_tmb_238','rad_tmb_340','rad_water_vapor','rad_cloud_liquid_water']
...
for g in gs:
    ...
    for product in swot_products:
        ...
        if (os.path.isfile(output1)==False) :
            save_swot_df = {k: [] for k in variables_to_save + (['x','y','x2','dist','unipix','avgtime','mode','cycle','pass_tile','file','avgtimestr','gauge'])}
    
            for pass_tile in passes_tiles:
                matching_passtiles = glob.glob(str(L2HR_folder/ ('%s_%s_C*_P%s_*%s*_*.nc' %(l2hr_prefix, resolution, pass_tile.split('_')[0], pass_tile.split('_')[1]))))
                print('\n\n\n[%s %s %s]\tPass-Tile: %s ' %(aoi, product, gauge_name, pass_tile))
                pass_dates = np.unique([matching_passtiles[i].split('/')[-1].split('_')[8] for i in range(len(matching_passtiles))])
                pass_dates.sort()
                print('\n[%s %s %s]\tPass-Tile: %s --> %s dates' %(aoi, product, gauge_name, pass_tile, len(pass_dates)))
                for date in pass_dates:
                    year = int(date[:4])
                    matches = sorted(glob.glob(str(L2HR_folder/ ('%s_%s_C*_P%s_*%s*_%s*.nc' %(l2hr_prefix, resolution, pass_tile.split('_')[0], pass_tile.split('_')[1], date)))))
                    if len(matches)>0:
                        print('\n[%s %s %s %s]' %(aoi, product, gauge_name, pass_tile))
                        print('Date: %s with %s matching files' %(date,len(matches)))
                        file = matches[-1]
                        fn = file.split('/')[-1]
                        cycle = fn.split('_')[5][1:]
                        
                        
                        
                        with xr.open_dataset(file,chunks={},engine='netcdf4') as ds:
                            df = pd.DataFrame({'x':ds.longitude.values.flatten(),'y':ds.latitude.values.flatten()})        
                            for variable in variables_to_save:
                                if len(ds[variable].values.shape)==2:                
                                    df[variable] = ds[variable].values.flatten()
                                else:
                                    print(variable)
                        df = df.dropna(subset = 'x').reset_index(drop=True)
                        df = df.dropna(subset = 'wse').reset_index(drop=True)
                        if len(df)>0:
                            df = fix_latlon(df)
                        ...
                        #     dist2gauge = haversine(g_lat,g_lon, df['y'][i], df['x'][i])
                        #     if dist2gauge <= search_radius_hr:
                        #         msk.append(i)        
                        #         dists.append(dist2gauge)
                        if len(df)>0:
                            df['dist'] = haversine_vectorized(g_lat, g_lon, df['y'], df['x2'])
                            df = df[df['dist'] <= search_radius_hr].copy()
                        ...
                        if len(df)>0:                        
                            ...
                            for var in variables_to_save + ['x','y','x2','dist']:
                                if var in ['time','time_tai']: 
                                    save_swot_df[var].extend(pd.to_datetime(df[var]).dt.tz_localize('UTC'))
                                elif var == 'geometry':
                                    save_swot_df[var].extend(df[var])
                                else:
                                    save_swot_df[var].extend(pd.to_numeric(df[var]))

                                
                            save_swot_df['cycle'].extend([cycle for i in range(0,len(df))])
                            save_swot_df['avgtime'].extend([pd.to_datetime(date) for i in range(0,len(df))])
                            save_swot_df['avgtimestr'].extend([pd.to_datetime(date).strftime('%Y%m%d %H%M') for i in range(0,len(df))])
    
                            save_swot_df['unipix'].extend([str(abs(a)) + str(abs(b)) for a,b in  zip(df['x'],df['y'])])
                            # save_swot_df['dist'].extend(df['dist'])
                            save_swot_df['mode'].extend([product for i in range(0,len(df))])
    
                            save_swot_df['file'].extend([file for i in range(0,len(df))])
                            save_swot_df['pass_tile'].extend([pass_tile for i in range(0,len(df))])
                            save_swot_df['gauge'].extend([gauge_name for i in range(0,len(df))])
            ...
            if len(save_swot_df['file'])>0:
                ...
                tmp = pd.DataFrame(np.column_stack([np.column_stack(list(save_swot_df.values()))]),columns=variables_to_save+(['x','y','x2','dist','unipix','avgtime','mode','cycle','pass','file','avgtimestr','gauge']))
                ...
                try:
                    watermask = gpd.read_file(output_dir / ('%s_watermask_200mbuffer.shp' %(aoi))).to_crs('4326').explode()
                    ...
                    joined = gpd.sjoin(save_swot_gdf, watermask, how='left', predicate='within')
                    save_swot_gdf[str(output_dir / ('%s_water_connected_10.shp' %(aoi)))] = save_swot_gdf.index.isin(joined[joined['index_right'].notna()].index)
                except:
